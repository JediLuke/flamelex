defmodule Flamelex.Buffer.Text do
  ...
  @impl GenServer
  def handle_call(:save, _from, %{source: {:file, filepath}} = state) do
    ...
    new_state =
      state |> Map.put(:unsaved_changes?, false)
  ...
  def handle_cast({:move_cursor, %{cursor_num: n, instructions: instructions} = details}, state) do
    case state.cursors |> Enum.at(n-1) do # fetch_cursor
      nil ->
          ...
          {:noreply, state}
      cursor ->
          new_cursor = move_cursor(cursor, instructions, state) # update the cursor
...
#     |> GenServer.cast({:move_cursor, instructions})
#   end
