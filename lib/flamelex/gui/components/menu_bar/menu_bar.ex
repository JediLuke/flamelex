defmodule Flamelex.GUI.Component.MenuBar do
  ...
  def init(scene, params, opts) do
    ...
    new_scene =
      ...
      |> assign(interaction_state: %{}) # holds internal state og the menu, e.g. %{hover: {1, 3}} means we've hovering over
      |> assign(state: :default) # holds internal state og the menu, e.g. %{hover: {1, 3}} means we've hovering over
      |> render_push_graph()
  ...
  def render_push_graph(scene) do
    new_scene = render(scene) # a pure function!!
  ...
  @impl Scenic.Scene
  def handle_input({:cursor_pos, {_x, _y} = coords} = input, _context, scene) do
    ...
    # end
    r = case coords |> hovering_over_item?() do
      {:main_menubar, index} ->
