defmodule Flamelex.GUI.Component.HyperCard do
  ...
  # def rego_tag(%{ref: ref}) do
  #   {:gui_component, {:hypercard, ref}}
  # end

  def init(scene, params, opts) do
    # ProcessRegistry.register(rego_tag(params)) #TODO ok so, should this component be by Memex id? Or somthing else (so we can update it??)
    # ProcessRegistry.register(:hypercard) #TODO ok so, should this component be by Memex id? Or somthing else (so we can update it?)
    Process.register(self(), :hypercard)
    
    #TODO this is where the Frame _SHOULD_ be specified!!
    # m = Flamelex.Memex.My.todos()
    # IO.inspect m
    # IO.inspect params.tidbit, label: "TdB"

    new_graph = 
        render(scene, params.tidbit)
