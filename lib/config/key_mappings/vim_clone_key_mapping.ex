defmodule Flamelex.Config.KeyMapping.VimClone do
  ...
  def binding(:vim_inspired_flamelex, %{mode: :normal, active_buffer: buf}) do #TODO we want to be bassing buffer structs around here...
    %{
      ...
      # leader keys
      leader() => %{
        @lowercase_k => {:apply_mfa, {Flamelex.CommandBufr, :show, []}}
      }
    }
  end
  ...
  """
  def lookup(%OmegaState{input: %{history: [last_key | _rest]}} = omega_state, input) do #NOTE: last key pressed was leader
